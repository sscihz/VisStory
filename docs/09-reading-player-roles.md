...
前面每章都故意只看一个问题：频率、责任、失误、篮板、效率、三分产量。最后一章的任务，是把这些单项问题重新拼回一个球员角色。
...
角色地图不是为了压缩所有篮球信息，而是为了给下一步分析排序。它告诉你应该先追问出手难度、传球职责、空间价值，还是防守和篮板。
...
把 Reaves 放进这套顺序后，读者应该避免一句话结论，而是形成类似这样的描述：

> 他在湖人进攻里承担一定回合终结，效率需要和同位置后卫比较；三分产量决定他是否稳定拉开空间，失误率决定持球任务的成本，篮板频率则补充他在回合回收上的角色。

这种描述比“他场均多少分”更接近篮球语境，也更容易和录像观察互相验证。
...
现代篮球分析不应该停留在“谁场均更多”。更好的问题是：谁在什么角色中，用多高频率、以多高效率、承担了怎样的进攻和回合责任。分布图给出定位，多指标组合给出角色，录像和战术背景负责解释原因。
