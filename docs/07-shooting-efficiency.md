...
![TS% 密度图：Austin Reaves 示例](assets/figures/07-ts-density-austin-reaves.svg)

这张图左侧比较不同位置的 3P%、eFG%、TS% 中位数，右侧直接看 TS% 密度分布，红线是 Austin Reaves。样本过滤为至少 500 分钟、至少 50 次三分出手。

这份真实数据里，TS% 的位置中位数大致在 **57% 左右**，锋线、后卫和中锋差距不大。但这不代表他们的进攻方式一样：有的人靠三分，有的人靠篮下和罚球，有的人靠中距离与高难度出手。TS% 把这些得分方式折算成整体效率，适合做第一层比较。

Austin Reaves 的 TS% 约 **64.1%**，在密度图里明显偏右。这说明如果只问“他的三分是不是特别顶级”，答案可能没那么夸张；但如果问“他把投篮和罚球合在一起的整体终结效率如何”，答案就更积极。这个例子正好说明：**三分命中率不是全部，TS% 能把罚球和两分价值一起纳入。**
