...
![篮板/36 分钟密度图：Austin Reaves 示例](assets/figures/06-rebounds-density-austin-reaves.svg)

左图是球员每 36 分钟篮板的密度图，红线是 Austin Reaves。它先回答一个最基础的问题：一个球员 36 分钟 5 个篮板，到底偏多还是偏少？

在 500+ 分钟样本中，中锋篮板中位数约 **12.0 个/36 分钟**，锋线约 **6.5**，后卫约 **4.8**。Austin Reaves 大约是 **4.9 个/36 分钟**，如果和全联盟所有位置比并不高，但如果按后卫职责看，就接近后卫的正常区间。这就是分布图和分位置图的意义：同一个数字，换一个位置参照系，解释会变。
