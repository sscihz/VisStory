...
![PACE 密度图：Lakers 示例](assets/figures/01-pace-density-lakers.svg)

上图使用 `surennba_stats` 的 30 支球队常规赛统计。左图是 **PACE 密度图**：红线标出 Lakers 在真实球队节奏分布中的位置，虚线是联盟中位数。这个图想说明：一个球队 pace 是 98、100 还是 103，单看数字很难判断快慢，必须先放回联盟分布。

本季球队节奏中位数大约在 99.7 回合/48 分钟，慢队和快队之间能差出接近 9 个回合。右图把 PACE 和进攻效率放在一起看：你会发现节奏快并不自动等于进攻好。
